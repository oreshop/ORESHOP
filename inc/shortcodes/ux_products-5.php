						...
						<div class="product-small <?php echo implode(' ', $classes_box); ?>">
							...
							<div class="box-text <?php echo implode(' ', $classes_text); ?>" <?php echo get_shortcode_inline_css($css_args); ?>>
								<?php
									...
									//////////// AFFICHAGE D'INFOS PLACEES SOUS LE PRIX DANS LES PAGES DE LISTE ////////////////
/////////// Récupération du champ personnalisé "infosup1" ////////////////
    $post_meta_array = get_post_meta($post->ID, 'infosup1');
    $nb_infos = count($post_meta_array);
    if ($nb_infos >= 1) {
        echo '<span class="infosup1">';
        echo get_post_meta($post->ID, 'infosup1', true);
        echo '</span>';
    }
/////////// Récupération du champ personnalisé "infosup2" ////////////////
		    $post_meta_array = get_post_meta($post->ID, 'infosup2');
		    $nb_infos = count($post_meta_array);
		    if ($nb_infos >= 1) {
		        echo '<span class="infosup2">';
		        echo get_post_meta($post->ID, 'infosup2', true);
		        echo '</span>';
		    }
/////////// Récupération du champ personnalisé "infosup3" ////////////////
				    $post_meta_array = get_post_meta($post->ID, 'infosup3');
				    $nb_infos = count($post_meta_array);
				    if ($nb_infos >= 1) {
				        echo '<span class="infosup3">';
				        echo get_post_meta($post->ID, 'infosup3', true);
				        echo '</span>';
				    }
/////////////////////////////////////////////////////////////////////
